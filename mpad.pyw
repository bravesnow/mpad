...
from element import menu,toolbar,window

def main():
    app=QtGui.QApplication(sys.argv)#应用程序实例化
    objwin = window.Window()#窗口实例化
    objmenu = menu.Menu(objwin)#菜单类实例化
    objtbar = toolbar.Toolbar(objwin)#工具栏实例化
    #信槽绑定
    QtCore.QObject.connect(objwin.textedit, QtCore.SIGNAL('textChanged()'),objwin.ontrans)
    #菜单信槽绑定
